...
public class ClientServerComms {
	...
	public void addSocksChannel(String socksRoutePath, F3<ISession, String, JsonObject> handler) {
		...
		sockJSHandler.socketHandler(ss -> {
			...
			SesssionWrapper sess = new SesssionWrapper(ss.webSession());
			ss.handler(b -> {
	...
	public void send(ISession session, String channelId, JsonObject data) {
		JsonObject msg = new JsonObject();
		msg.put("channelId", channelId);
		msg.put("data", data);
		SockJSSocket ss = this.socks.get(((SesssionWrapper)session).session);

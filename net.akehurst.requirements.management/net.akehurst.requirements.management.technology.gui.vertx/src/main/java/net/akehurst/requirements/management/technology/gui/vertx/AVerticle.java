...
public class AVerticle implements Verticle {
	...
	void addRoute(String stageId, Handler<RoutingContext> requestHandler, String webroot) {
...
//					holder.context = null;
				...
				this.ws.portGui().out(IAuthenticatorNotification.class).notifyAuthenticationSuccess(session);
			} else {
	...
	public void requestLogout(ISession session) {
		((SesssionWrapper)session).clearUser();
		this.ws.portGui().out(IAuthenticatorNotification.class).notifyAuthenticationCleared(session);
	}
